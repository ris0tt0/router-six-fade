...
export class ClientDbRpcImpl implements ClientDbRpc {
  ...
  async getPlayers(ids: UUID[]) {
    if (this.dbRpc) {
      ...
      switch (response.type) {
        case 'fail': {
          Logger.log('ws::dbClient::getPlayers', response.code, response.error);
          throw new Error(response.error);
        }
        case 'noResponse': {
          Logger.log('ws::dbClient::getPlayers no response');
          throw new Error('no response');
        }
        case 'success': {
          Logger.log('ws::dbClient::getPlayers', response.code, response.data);
          return response.data;
        }
      }
    }
  ...
  async updateGames(games: Game[]) {
    if (this.dbRpc) {
      ...
      switch (response.type) {
        ...
        case 'noResponse': {
          Logger.log('ws::dbClient::updateGames no response');
          throw new Error('no response');
        }
        case 'success': {
          Logger.log('ws::dbClient::updateGames', response.code, response.data);
          return response.data;
        }
      }
    }
  ...
  async getDatas(ids: UUID[]) {
    if (this.dbRpc) {
      ...
      switch (response.type) {
        case 'fail': {
          Logger.log('ws::dbClient::getDatas', response.code, response.error);
          throw new Error(response.error);
        }
        case 'noResponse': {
          Logger.log('ws::dbClient::getDatas no response');
          throw new Error('no response');
        }
        case 'success': {
          Logger.log('ws::dbClient::getDatas', response.code, response.data);
          return response.data;
        }
      }
    }
  ...
  async updateDatas(games: GameDataTypes[]) {
    if (this.dbRpc) {
      ...
      switch (response.type) {
        ...
        case 'noResponse': {
          Logger.log('ws::dbClient::updateDatas no response');
          throw new Error('no response');
        }
        case 'success': {
          Logger.log('ws::dbClient::updateDatas', response.code, response.data);
          return response.data;
        }
      }
    }
